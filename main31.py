...
def get_gap(player_cp_times, computer_cp_times):
    if len(player_cp_times) == 0 or len(computer_cp_times) == 0:
        return 0.0, (255, 255, 255)  # No gap if no checkpoints reached
    
    last_cp_index = min(len(player_cp_times), len(computer_cp_times)) - 1
    gap = player_cp_times[last_cp_index] - computer_cp_times[last_cp_index]

    # Add time since last checkpoint for the car that's ahead
    current_time = Game.get_level_time()
    if len(player_cp_times) > len(computer_cp_times):
        # Player is ahead - add time since computer's last checkpoint
        gap += (current_time - computer_cp_times[-1])
    elif len(computer_cp_times) > len(player_cp_times):
        # Computer is ahead - subtract time since player's last checkpoint
        gap -= (current_time - player_cp_times[-1])
...
def draw_HUD(win):
    ...
    blit_info_box(win, "HUD", f"Time: {level_time} s", 10, win.get_height()-40)
    gap, colour = get_gap(player_car.cp_times, computer_car.cp_times)
    if gap >= 0:
        gap = str("+" + str(gap))
    blit_info_box(win, "HUD", f"Gap: {gap} s", 10, win.get_height()-80, text_color= colour)
...
computer_car = ComputerCar(2, 3.5, PATH) # instantiating the computer car
