...
class TextBox:
    # Class defaults (shared by all textboxes)
    WIDTH = 240
    HEIGHT = 32
    PADDING = 6

    def __init__(self, x, y, font, is_password = False):
        ...
        self.text = ""
        self.active = False
        self.txt_surf = font.render("", True, (255, 255, 255))
    ...
    def _update_text_surface(self):
        max_width = self.rect.w - self.PADDING * 2
        
        # Display asterisks for password field
        display_text = "*" * len(self.text) if self.is_password else self.text
        
        text = display_text
        while self.font.size(text)[0] > max_width:
            text = text[1:]
    ...
    def draw(self, surf):
        ...
        text_y = self.rect.y + (self.rect.h - self.txt_surf.get_height()) // 2

        surf.blit(self.txt_surf, (text_x, text_y))
