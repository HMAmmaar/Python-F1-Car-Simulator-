...
def obtain_security_question(username: str, path: str = USERS_FILE) -> str:
    """
    Retrieve the security question for a given username.
    
    Args:
        username: The username to look up
        path: Path to the users JSON file
        
    Returns:
        The security question string if user exists and has a question
        None if user doesn't exist or has no security question
    """
    ...
    # Get security question, return None if not found
    return user_data.get('sec_question')
