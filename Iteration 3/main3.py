...
from utils import scale_image, blit_rotate_center, blit_info_box
from tracks import TrackManager, Track, track_1, track_2
...
WORLD_WIDTH, WORLD_HEIGHT = 1200, 1200  # Big enough for your largest track
...
track_manager.add_track(track_1)
print(f"✅ Current track: {track_manager.get_current_track().name}")

# Debug: Check if track images loaded
current_track = track_manager.get_current_track()
print(f"Track image size: {current_track.track.get_size() if current_track.track else 'FAILED TO LOAD'}")
print(f"Track border size: {current_track.track_border.get_size() if current_track.track_border else 'FAILED TO LOAD'}")
print(f"Grass size: {current_track.grass.get_size() if current_track.grass else 'FAILED TO LOAD'}")
print(f"Finish size: {current_track.finish.get_size() if current_track.finish else 'FAILED TO LOAD'}")
...
class AbstractCar:
    ...
    def draw_checkpoints(self, win):
        if DEBUG:
            for point in track_manager.get_current_track().path:
                pygame.draw.circle(win, (255,0,0), point , 5) # draw red circle at each point in path

    def update_checkpoint(self):
        target = track_manager.get_current_track().path[self.current_point] # get the current target point
        ...
        if DEBUG:
            ...
            pygame.draw.circle(WORLD, (0,255,0), track_manager.get_current_track().path[self.current_point] , 10, width=1) 
...
class PlayerCar(AbstractCar):
    IMG = RED_CAR
    START_POS = track_manager.get_current_track().player_start_pos
...
class ComputerCar(AbstractCar):
    IMG = GREEN_CAR
    START_POS = track_manager.get_current_track().comp_start_pos

    def __init__(self, max_vel, rotation_vel):
        super().__init__(max_vel, rotation_vel) # call the init method of the AbstractCar class
        self.path = track_manager.get_current_track().path # list of points the computer car will follow
...
def sector_times():
    total_path_length = len(track_manager.get_current_track().path)
...
images = [(track_manager.get_current_track().grass, (0, 0)), (track_manager.get_current_track().track, (0, 0)), (track_manager.get_current_track().finish, track_manager.get_current_track().finish_pos), (track_manager.get_current_track().track_border,(0,0))] # list of images to be drawn on the screen
player_car = PlayerCar(3, 3.5) # instantiating the player car passing in max velocity and rotation velocity
computer_car = ComputerCar(2.5, 3.5) # instantiating the computer car
...
while run:
    ...
    for event in pygame.event.get(): # event loop
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            computer_car.path.append(world_pos)
    ...
    if player_car.collide(track_manager.get_current_track().track_border_mask)!= None: # check if car collides with track border
        player_car.bounce() # if it does, bounce the car back
    
    if computer_car.collide(track_manager.get_current_track().finish_mask, *track_manager.get_current_track().finish_pos) != None: # check if car collides with finish line
        if computer_car.current_point >= len(track_manager.get_current_track().path)-1: # check if car has passed all checkpoints
    ...
    if player_car.collide(track_manager.get_current_track().finish_mask, *track_manager.get_current_track().finish_pos) != None: # check if car collides with finish line
        if player_car.current_point >= len(track_manager.get_current_track().path)-1: # check if car has passed all checkpoints
